function sketch() {
	...
	function getRandomColor() {
		...
		randomColor = 'rgb(' + getRandomRGBNumber() + ', ' + getRandomRGBNumber() + ', ' + getRandomRGBNumber() + ')';
		return randomColor;
	}
	...
	function getUserGrid() {
		let userGrid = +prompt('Enter the number of pixels in single row or column (max. 100):');
		if (userGrid<0 || userGrid>100 || !(Number.isInteger(userGrid)) || !(typeof userGrid === 'number')) {
			while ((!(userGrid>0 && userGrid<100 && Number.isInteger(userGrid) && (typeof userGrid === 'number')))) {
				userGrid = +prompt('Enter a whole number beetween 1 and 100:');
			}
			return userGrid;
		} else {
		return userGrid;
